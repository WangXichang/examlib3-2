...
class TZscoreLinear(ScoreTransformModel):
    ...
    def set_parameters(self, rscore_max=150, rscore_min=0, tscore_std=10, tscore_mean=50, tscore_stdnum=4):
        self.rscore_max = rscore_max
        self.rscore_min = rscore_min
        self.tscore_mean = tscore_mean
        self.tscore_std = tscore_std
        self.tscore_stdnum = tscore_stdnum
    ...
    def run(self):
        ...
        self.outdf = self.rawdf[self.scorefields]
        for sf in self.scorefields:
            rmean, rstd = self.outdf[sf].describe().loc[['mean', 'std']].valuse[:, 0]
            self.outdf[sf+'_zscore'] = \
