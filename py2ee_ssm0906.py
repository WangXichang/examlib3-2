...
class ScoreTransformModel(object):
    ...
    def run(self):
        if (not self.check_data()) | (not self.check_parameter()):
            return
    ...
    def __plotoutscore(self):
        ...
        for sf in self.scorefields:
            self.outdf.groupby(sf + '_'+self.modelname)[sf + '_'+self.modelname].count().plot(label=sf)
        return
    ...
    def __plotrawscore(self):
        ...
        for sf in self.scorefields:
            self.rawdf.groupby(sf)[sf].count().plot(label=sf)
        return
...
class Zscore(ScoreTransformModel):
    ...
    def run(self):
        ...
        for sf in self.scorefields:
            ...
            self.outdf[sf+'_z'] = self.outdf[sf].replace(self._segtable.seg.values,
                                                         self._segtable[sf+'_zscore'])
