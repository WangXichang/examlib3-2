    ...
    def run(self):
        ...
        for f in self.__segFields:
            ...
            if self.__segStep > 1:
                ...
                c = 0
                if self.__segSort == 'ascending':
                    curpoint = self.__segMin; curstep = self.__segStep
                else:
                    curpoint = self.__segMax; curstep = -self.__segStep
...
def exp_segtable():
    df = pd.DataFrame({'a':[x for x in range(10000)], \
                       'b':[int(np.random.randn()*100) for x in range(10000)]})
    seg = segcount()
    seg.inputdf = df
